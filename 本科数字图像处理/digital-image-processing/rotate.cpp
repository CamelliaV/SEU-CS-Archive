...
void Rotate::rotateIndex8(QImage *&target, QImage *reference)
{
...
void Rotate::rotateGray8(QImage *&target, QImage *reference)
{
  if (!target || !reference) return;
  double angle = ui->value->value();
  double radian = angle * M_PI / 180;
  ...
  QImage *image = new QImage(QSize(cols, rows), QImage::Format_Grayscale8);
   ...
   for (int y = 0; y < rows; y++) {
     uchar* scanner = image->scanLine(y);
     for (int x = 0; x < cols; x++) {
       ...
       if (pos_y == yy && pos_x == xx) {
           // image->setPixel(x, y, reference->pixelIndex(xx, yy));
           scanner[x] = reference->scanLine(yy)[xx];
         // newMat[y][x] = mat[yy][xx];
         continue;
       }
       ...
       double Q_00 = (1 - u) * (1 - v) * reference->scanLine(yy)[xx];
       double Q_10 = (1 - u) * (v) * reference->scanLine(yy + 1)[xx];
       double Q_01 = (u) * (1 - v) * reference->scanLine(yy)[xx + 1];
       double Q_11 = (u) * (v) * reference->scanLine(yy + 1)[xx + 1];

       scanner[x] = Q_00 + Q_01 + Q_10 + Q_11;
     }
...
void Rotate::rotateHandler(QImage *&target, QImage *reference)
{
  ...
  if (reference->format() == QImage::Format_Grayscale8) return rotateGray8(target, reference);
