...
void Scale::scaleIndex8(QImage *&target, QImage *reference)
{
  if (!target) return;
...
void Scale::scaleGray8(QImage *&target, QImage *reference)
{
  if (!target) return;
  // qDebug() << target->width() << " " << target->height();
  int rows = reference->height();
  ...
  int newRows = int(rows * scale);
  int newCols = int(cols * scale);
  QImage *image = new QImage(QSize(newCols, newRows), QImage::Format_Grayscale8);
  ...
  for (int i = 0; i < newRows; i++) {
      ...
      for (int j = 0; j < newCols; j++) {
        ...
        if (pos_i == pos_i_integer && pos_j == pos_j_integer) {
          scanner[j] = reference->scanLine(pos_i_integer)[pos_j_integer];
          // image->setPixel(j, i, reference->pixelIndex(pos_j_integer, pos_i_integer));
          // newMat[i][j] = mat[pos_i_integer][pos_j_integer];
          continue;
        }
...
//        double Q_11 = (u) * (v) * mat[pos_i_integer + 1][pos_j_integer + 1];
        ...
        double Q_00 = (1 - u) * (1 - v) * reference->scanLine(pos_i_integer)[pos_j_integer];
        double Q_10 = (1 - u) * (v) * reference->scanLine(pos_i_integer + 1)[pos_j_integer];
        double Q_01 = (u) * (1 - v) * reference->scanLine(pos_i_integer)[pos_j_integer + 1];
        double Q_11 = (u) * (v) * reference->scanLine(pos_i_integer + 1)[pos_j_integer + 1];

        // newMat[i][j] = Q_00 + Q_01 + Q_10 + Q_11;
        scanner[j] = Q_00 + Q_01 + Q_10 + Q_11;
      }
    }
    delete target;
    target = image;
...
void Scale::scaleHandler(QImage *&target, QImage *reference)
{
  ...
  if (reference->format() == QImage::Format_Grayscale8) return scaleGray8(target, reference);
